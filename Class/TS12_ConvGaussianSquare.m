...
for k=1:3
	...
	n = n+1;
	H=figure(n);
	set(H,'Name','Filtrado por convolución con un cuadrado: equivalente a un filtrado pasa baja')
	subplot(2,3,1)
		imagesc(f);
		axis equal tight
...
for k=1:3
	...
	n = n+1;
	H=figure(n);
	set(H,'Name','Filtrado por convolución con una guasiana: equivalente a un filtrado pasa baja')
	subplot(2,3,1)
		imagesc(f);
		axis equal tight
	...
	subplot(2,3,2)
		imagesc(r);
		titulo = sprintf('Gausiana de ancho %d', 2*d+1);
		title(titulo)
		axis equal tight
	...
	subplot(2,3,3)
		colormap(gray);
		imagesc(c );
		title('Convolución con gausiana')
		axis equal tight
